...
export const httpFormat = (req: Request, res: Response, type: 'request' | 'response') => {
  const {
    method,
    url,
    originalUrl,
...
export const backendFormat = printf(({ level, message, timestamp, ...rest }) => {
  ...
  Object.entries({
    ...
    context,
    errorCode,
    stacktrace: stacktrace && (stacktrace as any).replaceAll('\n', ''),
  }).forEach(([key, value]) => {
    if (value) {
      if (key === 'context' && typeof value !== 'string') {
        value = JSON.stringify(value);
