...
import fs from 'fs';
import { isPrivateApi } from './privateUrl';
...
export const logInfo = ({
  message,
  context,
}: Pick<LogRecordBase, 'message' | 'context'>) => {
...
export const netLogs = (app: Application) => {
  app.use((req: Request, res: Response, next: NextFunction) => {
    ...
    if (isPrivateApi(req.url)) logger.api(httpFormat(req, res, 'request'));
    res.on('finish', () => {
      logger.http(httpFormat(req, res, 'response'));
      if (isPrivateApi(req.originalUrl))
        logger.api(httpFormat(req, res, 'response'));
    });
    next();
  });
};
